...
def send_welcome(message):
    bot.send_message(message.chat.id,
                     "Бот помогает определить, сколько побед в день нужно для получения медали 2 сезона премьера в CS2.")
...
def check_medal_progress(message):
    ...
    if remaining_wins <= 0:
        ...
        wins_text = "побед" if remaining_wins % 10 in [0, 5, 6, 7, 8, 9] or (
                    10 <= remaining_wins % 100 <= 20) else "победы" if remaining_wins % 10 in [2, 3, 4] else "победа"
        days_text = "дней" if remaining_days % 10 in [0, 5, 6, 7, 8, 9] or (
                    10 <= remaining_days % 100 <= 20) else "дня" if remaining_days % 10 in [2, 3, 4] else "день"
